...
//  appconfig wurde globalisiert das bedeutet wenn es benützt wird braucht es den selben inhalt wie im 
//interface AppConfig hier fehlen jedoch das debug und maxRetries.
//
...
// gegenüber any?
//
// Antwort: any sagt typescript vertrau mir du must hier den code nicht überprüfen, 
//das kann zu vielen fehlern führen.  
...
//
// Erwartung:
// 1. Reihenfolge der Ausgaben: nach promise konstruktion fehler abgefangen,boom im executer 
// 2. Warum führt der throw nicht zum Absturz? Im promise ist ein eingebautes catch das den fehler abfängt und weiterleitet an die .then() methode.
// 3. Was passiert mit dem Error-Objekt intern?  Der error hat das selbe ergebnis wie wenn man reject hinschreibt und wird mit catch abgefangen.
...
// Antwort: Der einzige unterschied is die schreibweise, eines mit throw das andere mit reject führt aber zum selben ergebnis.
...
// 1. Wird das Promise fulfilled oder rejected? fullfilled
// 2. Was passiert mit dem throw? es wird ignoriert da ein promise nur einmal settlen kann und resolve zuerst ist.
// 3. Wird die Error-Nachricht irgendwo sichtbar? nein weil diese nach dem resolved steht und alles danach wird irgnoriert.
...
// Antwort:
//ignoriert weil nur einmal settled
// ============================================================================
...
// Antwort:
//  ja weil die funktipn im promise aufgerufen wird und sozusagen als reject behandelt wird undsomit auch gecatcht wird.
...
// Antwort:
//Man kann besser vefolgen was als nächstes passiert und hat mehr kontrolle über die fehlerbehandlung.
// ============================================================================
...
function fetchData(): Promise<string> { return new Promise((resolve, reject) => {
    throw new Error("Netzwerkfehler");
  });
}
...
// Antwort: Eine funktion hört nah einen return auf man steigt aus der funktion aus. Alles unter return wird ignoriert.
// also es wird das ergebnis ausgegeben und danach werden die sachen auserhalb der funktion weiter ausgeführt.
...
async function withRetry<T>(
  ...
  while (globalThis.__retryCount < maxRetries) {
  try {
...
throw new Error("Keine Versuche mehr übrig");
}
// Test:
...
//
// Antwort:
// Eine setTimeout funktion ist asynchron das bedeutet sie passiert später promise sind sinchron das bedeutet sie werden direkt ausgeführt.
// Das throw wird also erst später ausgeführt somit wir resolve zuerst erkannt und settled somit das promise zuerst. 
// Deswegen muss man nichts catchen. Ein throw im executor wird direkt mit catch gefangen da es synchron ist. Im setTimout
// wird es nicht mehr mit catch aufgefangen weil und somit nicht mehr mit dem promise zu tun hat.